...
| `models/qwen35/adaptive-scheduler-policy.md` | Issue #727 adaptive scheduler: default `off` / opt-in `auto`, hard prefill cap, TP rejects `auto`; 4090 in-process + HTTP QPS A/B complete (`ded5fee` / task `650826`) — auto≈off on std/longout, mixed ITL_STEP gate holds, QPS out tok/s −3–6% reported. |
